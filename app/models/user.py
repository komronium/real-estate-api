...
from sqlalchemy import Column, UUID, String, Boolean, Enum as SQLAlchemyEnum, DateTime, Date, ForeignKey
...
class OneIDInfo(Base):
    ...
    pin = Column(String(length=14), unique=True, nullable=False, comment="Personal identification number (JShShIR)")
    one_id_user_id = Column(String(length=255), unique=True, nullable=False, comment="One ID user identifier")
    one_id_session_id = Column(String(length=255), nullable=True, comment="One ID session identifier")
    
    # Shaxsiy ma'lumotlar
    full_name = Column(String(length=255), nullable=False, comment="Full name from One ID")
    first_name = Column(String(length=255), nullable=False, comment="First name from One ID")
    last_name = Column(String(length=255), nullable=False, comment="Last name from One ID")
    middle_name = Column(String(length=255), nullable=False, comment="Middle name (father's name) from One ID")
    ...
    auth_method = Column(String(length=20), nullable=True, comment="Authentication method")
    
    # Yuridik shaxs ma'lumotlari (agar bo'lsa)
    pkcs_legal_tin = Column(String(length=9), nullable=True, comment="Legal entity TIN for PKCS auth")
