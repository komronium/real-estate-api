...
class OneIDUserInfo(BaseModel):
    ...
    validation_method: Optional[List[str]] = Field(None, description="Verification methods used")
    ...
    mid_name: str = Field(..., description="Middle name (father's name)", max_length=255)
    user_type: str = Field(..., description="User type (I-Individual, L-Legal)", max_length=1)
    sess_id: str = Field(..., description="Session ID (UUID)")
    ret_cd: str = Field(..., description="Return code (0-success, 1-failure)", max_length=1)
    auth_method: str = Field(..., description="Authentication method", max_length=20)
    pkcs_legal_tin: Optional[str] = Field(None, description="Legal entity TIN for PKCS auth", max_length=9)
    legal_info: Optional[List[OneIDLegalInfo]] = Field(None, description="Legal entity information")
...
class OneIDInfoResponse(BaseModel):
    ...
    one_id_session_id: Optional[str] = Field(None, description="One ID session identifier")
...
class UserWithOneIDResponse(BaseModel):
    """User response with One ID information"""
